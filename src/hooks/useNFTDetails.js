...
const useNFTDetails = () => {
    ...
    const getNFTDetails = async (id) => {
        ...
        try {
            ...
            const { type, fields, owner: { AddressOwner, ObjectOwner } } = data;
            //const { data: { type, fields }, owner: { AddressOwner, ObjectOwner }, } = details;
            //NFT is on market
            if (ObjectOwner) {
                const dataWrap = await getObject(ObjectOwner);
                const { data: details } = await getObject(dataWrap?.data?.owner?.ObjectOwner);
                if (!details)
                    return result
            ...
            return {id, owner: AddressOwner, typeNFT: type, fields, isListed: false }
        } catch (ex) {
            console.log(ex)
        }
        return result;
